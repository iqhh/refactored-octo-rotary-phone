#!/usr/bin/env python3
"""
Intelligent AI Gomoku - Human Level (Fixed)
Features: Deep thinking (8 plies), advanced threat detection, winning strategy recognition
FIX: Proper defense against active three, better threat prioritization
"""
...
class AdvancedGomokuAI:
    ...
    def get_best_move(self, board: GomokuBoardAdvanced) -> Optional[Tuple[int, int]]:
        ...
        # Phase 2.5: CRITICAL - Defend all active threat patterns
        move = self._defend_threats(board)
        if move:
            return move
    ...
    def _detect_threats(self, board: GomokuBoardAdvanced, player: Player) -> List[Dict]:
        """
        Detect all threats for a player.
        Returns list of threat dictionaries with keys:
        - type: '4' (活四), 'blocked_4', '3' (活三), 'blocked_3', etc.
        - positions: list of affected board positions
        - openings: list of empty positions that need blocking
        """
        threats = []
        directions = [(0, 1), (1, 0), (1, 1), (1, -1)]
        char = 'X' if player == Player.AI else 'O'
        opp_char = 'O' if player == Player.AI else 'X'
            ...
            for col in range(board.size):
                if board.board[row][col] == player:
                    for dr, dc in directions:
                        ...
                        # 活四: _XXXX_
                        if f'_{char*4}_' in line:
                            idx = line.index(f'_{char*4}_')
                            left_pos = idx
                            right_pos = idx + 5
                            r1, c1 = row + (left_pos - center_pos) * dr, col + (left_pos - center_pos) * dc
                            r2, c2 = row + (right_pos - center_pos) * dr, col + (right_pos - center_pos) * dc
                            threats.append({
                                'type': '4',
                        ...
                        # 活三: _XXX_
                        if f'_{char*3}_' in line:
                            idx = line.index(f'_{char*3}_')
                            left_pos = idx
                            right_pos = idx + 4
                            r1, c1 = row + (left_pos - center_pos) * dr, col + (left_pos - center_pos) * dc
                            r2, c2 = row + (right_pos - center_pos) * dr, col + (right_pos - center_pos) * dc
                        ...
                        # 间隔活三: _X_XX_, _XX_X_
                        patterns = [f'_{char}_{{2}}{char}_', f'_{char}_{char}_{char}_']
                        for i, pattern_template in enumerate([f'_{char}_XX_', f'_XX_X_']):
                            if pattern_template in line:
                                idx = line.index(pattern_template)
                                for j, ch in enumerate(pattern_template):
                                    if ch == '_':
                                        pos = idx + j
                                        r, c = row + (pos - center_pos) * dr, col + (pos - center_pos) * dc
    ...
    def _defend_threats(self, board: GomokuBoardAdvanced) -> Optional[Tuple[int, int]]:
        """
        Intelligently defend against opponent's threats.
        Priority:
        1. Block opponent's active 4 (guaranteed win next move)
        2. If opponent has multiple threats, find move that blocks most threats
        3. If own move creates threat, prioritize creating own opportunities
        """
        opponent_threats = self._detect_threats(board, Player.HUMAN)
        
        if not opponent_threats:
            return None
        
        # Sort by severity
        opponent_threats.sort(key=lambda x: x['severity'], reverse=True)
        
        # Get all blocking positions
        blocking_candidates = set()
        for threat in opponent_threats:
            for pos in threat['positions']:
                if board.is_valid_move(pos[0], pos[1]):
                    blocking_candidates.add(pos)
        ...
        for block_row, block_col in blocking_candidates:
            # Try this blocking move
            board.board[block_row][block_col] = Player.AI
            board.last_move = (block_row, block_col)
            
            # Evaluate:
            # 1. How many opponent threats does it block?
            remaining_threats = self._detect_threats(board, Player.HUMAN)
            blocked_count = len(opponent_threats) - len(remaining_threats)
            
            # 2. Does it create our own threat?
            our_threats = self._detect_threats(board, Player.AI)
            our_threat_severity = sum(t['severity'] for t in our_threats)
            
            # Score: prioritize blocking, but reward creating our own threats
            score = blocked_count * 50000 + our_threat_severity
    ...
    def _find_double_threat(self, board: GomokuBoardAdvanced) -> Optional[Tuple[int, int]]:
        ...
        for row, col in board.get_valid_moves():
            ...
            threats = self._detect_threats(board, Player.AI)
            board.board[row][col] = Player.EMPTY
